...
//swapped x&y for rot 90 deg
//line 337 buggery too
stemWid = 6;
stemLen = 8;
stemCrossHeight = 1.0; //1.8 default
extra_vertical = 4.6;
stemLayers = 50; //resolution of stem to cap top transition 50 default
...
function StemTranslation(t, keyID) =
  [
    0,   //X shift
    0,   //Y shift
    0.8+stemCrossHeight+.1 + (t/stemLayers*(KeyHeight(keyID)- topthickness - stemCrossHeight-.1))    //Z shift
  ];
...
//          cube([12,8,3]);    
//          } //HACK JKL, ugh maybe the worst hack of this iteration.. cube replaces plane that was commented out above, so keycap has even surface to press agianst top of keyswitch when depressed
...
module choc_stem(draftAng = 5) {
  ...
  dia = .15;
  wids = 1.3/2; //1.2/2 default
  lens = 2.8/2; //2.9/2 default
  module Stem() {
    difference(){
